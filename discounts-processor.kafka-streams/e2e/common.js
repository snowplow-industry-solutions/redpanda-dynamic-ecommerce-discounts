...
export async function createSummaryFile(events, testName) {
  ...
  // First pass: collect all events by product and calculate delays
  events.forEach(event => {
    const productId =
      event.event_name === 'product_view'
        ? event.product_id
        : 'product' + event.webpage_id.replace('page', '')

    const currentTimestamp = new Date(event.collector_tstamp).getTime()

    // Initialize or update product summary
    const existingProduct = productSummary.get(productId)
    if (existingProduct) {
      if (event.event_name === 'product_view') {
        existingProduct.views++
        existingProduct.pingCount.push(0) // Inicia com 0 para nova visualização
      } else if (event.event_name === 'page_ping') {
        // Incrementa o último contador de pings
        if (existingProduct.pingCount.length > 0) {
          existingProduct.pingCount[existingProduct.pingCount.length - 1]++
        } else {
          existingProduct.pingCount.push(1)
        }
      }
      existingProduct.lastView = event.collector_tstamp
      existingProduct.durationInSeconds = config.durationInSeconds(
        existingProduct.pingCount.reduce((sum, count) => sum + count, 0)
      )
    } else {
      productSummary.set(productId, {
        productId: productId,
        productName: event.event_name === 'product_view' ? event.product_name : 'Unknown',
        views: event.event_name === 'product_view' ? 1 : 0,
        durationInSeconds: 0,
        pingCount: [0], // Inicia com [0] para primeira visualização
        lastView: event.collector_tstamp,
        delayToNextProduct: []
      })
    }

    // Calculate delay if this is a new product view
    if (
      event.event_name === 'product_view' &&
      lastProductEndTime &&
      lastProductId &&
    ...
    // Update last product info
    if (event.event_name === 'product_view' || event.event_name === 'page_ping') {
      lastProductId = productId
      lastProductEndTime = currentTimestamp
    }
  })

  // Processa o último produto
  const lastProduct = productSummary.get(lastProductId)
  ...
  const summaryArray = Array.from(productSummary.values()).sort((a, b) => {
    if (b.durationInSeconds !== a.durationInSeconds) {
  ...
  const summaryFile = path.join(__dirname, `data/${testName}.summary.json`)
