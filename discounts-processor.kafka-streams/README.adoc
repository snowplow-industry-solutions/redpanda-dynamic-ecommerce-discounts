...
This project (*{doctitle}*) is a Java/ {KafkaStreams} application that processes event data from {Redpanda} and generates <<output-events,discount events>> based on some <<rules,conditions>>.
